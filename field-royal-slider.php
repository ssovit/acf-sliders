<?php
class WPPress_RoyalSlider_ACF_Field extends acf_field
{
	
	function __construct() {
		$this->name = 'wppress_acf_royalslider';
		$this->label = __('Royal Slider');
		parent::__construct();
	}
	...
	function format_value_for_api($value, $field) {
		...
		if (is_array($value)) {
			foreach ($value as $k => $v) {
				$f = get_new_royalslider( $v );
				$value[$k] = array();
				$value[$k] = $f;
			}
		} else {
			$value = get_new_royalslider( $value);
		}
	...
	function create_field($field) {
		...
		$qstr = $wpdb->prepare("SELECT id, name FROM $table WHERE active=1  AND type!=%s",'gallery');
		$sliders = $wpdb->get_results( $qstr );
		...
		if ($sliders) {
			foreach ($sliders as $i=>$item) {
				...
				if (is_array($field['value'])) {
					if (in_array($item->id, $field['value'])) {
